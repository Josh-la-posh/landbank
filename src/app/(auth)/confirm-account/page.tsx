...
const onSubmit = async (values: Values) => {
  ...
  try {
    ...
    if (!data) {
      setError(error || 'Unable to confirm account right now.');
      return;
    }

    if (!data.requestSuccessful) {
      setError(data.message || error || 'Unable to confirm account right now.');
      return;
    }
...
<div>
<label className="label">Confirmation token</label>
